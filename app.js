(() => {
  ...
  const GROUP_ORDER = ['Triunviro','Dii Consentis','Dii Inferi','Ventis','Numina','Alati','Outros'];
  const GROUP_LABEL = {
    'Triunviro':'Triunviros','Dii Consentis':'Dii Consentis','Dii Inferi':'Dii Inferi',
    'Ventis':'Ventis','Numina':'Numina','Alati':'Alati','Outros':'Outros'
  };
  ...
  const defaultArmor = () => ({equipped:false,name:'Armadura',type:'nenhuma',material:'ferro-aco',resistanceCurrent:3});
  const defaultShield = () => ({equipped:false,name:'Escudo',material:'ferro-aco',stakes:0,resistanceCurrent:3,masterTalent:false});
  ...
  function automaticSkillSources(name){
    ...
    if(talentGrantedSkills().includes(name))out.push('Talento');
  ...
  function skillHasExpertise(name){return skillIsProficient(name)&&!!skillMetaFor(name).expertise}
  ...
  function save(){state.schemaVersion=SCHEMA_VERSION;state.updatedAt=new Date().toISOString();localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}
  ...
  function migrate(data){
    ...
    out.inventory.items=Array.isArray(data?.inventory?.items)?data.inventory.items.map(it=>Object.assign({id:uid('item'),name:'Item',qty:1,category:'geral',material:'',rune:'',notes:''},it,{qty:Math.max(0,Number(it?.qty)||0)})):[];
    ...
    out.weapons=Array.isArray(data?.weapons)?data.weapons.map(w=>Object.assign({id:uid('weapon'),name:'Arma',type:'corpo-a-corpo',attr:'for',material:'ferro-aco',stakes:0,resistanceCurrent:3,attackExtra:0,damageExtra:0,equipped:true},w)):[];
    out.armor=Object.assign(defaultArmor(),data?.armor||{});
    out.shield=Object.assign(defaultShield(),data?.shield||{});
    out.activeTab=['status','combat','abilities','inventory','familiars','roma','magic','history','notes'].includes(data?.activeTab)?data.activeTab:(data?.activeTab==='crafting'?'inventory':'status');
  ...
  function changeBase(k,delta){const cur=state.baseAttributes[k]||0;if(delta>0&&(sum(state.baseAttributes)>=8||ordinaryAttrRaw(k)>=5))return;if(delta<0&&cur<=0)return;state.baseAttributes[k]=cur+delta;const limit=initialSkillLimit();if(state.initialSkills.length>limit)state.initialSkills=state.initialSkills.slice(0,limit);save();renderCreation()}
  ...
  function renderTabs(){
    const tabs=[['status','Status'],['combat','Combate'],['abilities','Habilidades'],['inventory','Inventário'],['familiars','Familiares'],['roma','Roma'],['magic','Magia'],['history','História'],['notes','Notas']];
    return `<nav class="sheet-tabs">${tabs.map(([id,label])=>`<button type="button" data-tab="${id}" class="${state.activeTab===id?'active':''}">${label}</button>`).join('')}</nav>`;
  }
  ...
  function renderCombatResourceDock(){
    ...
    return `<section class="combat-resource-dock">
      ...
      ${g.resource?`<div class="combat-resource-chip special"><span>${g.resource.name}</span><strong>${state.resourceCurrent}<small>/${rMax}</small></strong><div class="combat-resource-adjust special-buttons"><button type="button" data-resource="-1">−1</button><button type="button" data-resource="1">+1</button></div></div>`:''}
    </section>`;
  }
  ...
  function validationIssues(){
    ...
    if(sum(state.baseAttributes)!==8)add('error',`A criação usa exatamente 8 pontos de atributo; esta ficha tem ${sum(state.baseAttributes)}.`);
  ...
  function renderStatusTab(g){
    ...
    return `<section class="tab-pane ${state.activeTab==='status'?'':'hidden'}" data-pane="status">
      ${renderResourceDock()}
      <div class="grid three status-top-grid">
        <article class="card profile-edit-card"><p class="eyebrow">PERFIL</p><h2>Identidade & mídia</h2><div class="stack"><label><span class="label">Personagem</span><input id="sheetNameInput" value="${esc(state.name)}"></label><div class="grid two compact-fields"><label><span class="label">Player</span><input id="sheetPlayerInput" value="${esc(state.player)}"></label><label><span class="label">Nível</span><input id="levelInput" type="number" min="1" max="100" value="${state.level}"></label></div><label><span class="label">Frase / subtítulo do perfil</span><input data-history="tagline" value="${esc(h.tagline||'')}" placeholder="Ex.: Centurião da Segunda Coorte"></label><div class="grid two compact-fields"><label><span class="label">Idade</span><input data-history="age" value="${esc(h.age||'')}" placeholder="Opcional"></label><label><span class="label">Nascimento</span><input data-history="birth" value="${esc(h.birth||'')}" placeholder="Data ou texto livre"></label></div><div class="grid two compact-fields"><label><span class="label">Origem</span><input data-history="origin" value="${esc(h.origin||'')}" placeholder="Cidade, região..."></label><label><span class="label">Residência / afiliação</span><input data-history="residence" value="${esc(h.residence||h.affiliation||'')}" placeholder="Nova Roma, coorte, grupo..."></label></div><label><span class="label">URL da foto de perfil</span><input data-history="portraitUrl" value="${esc(h.portraitUrl||'')}" placeholder="Cole uma URL de imagem"></label><label><span class="label">URL do banner</span><input data-history="bannerUrl" value="${esc(h.bannerUrl||'')}" placeholder="Cole uma URL de imagem"></label><div class="subcard compact"><b>${g.name}</b><br><span class="muted">${GROUP_LABEL[g.group]||g.group} · BP +${bp()} · Conjuração ${attrName(g.casting)}</span></div></div></article>
        <article class="card wide-status"><p class="eyebrow">ATRIBUTOS</p><div class="attrs">${ATTRS.map(a=>attrCard(...a,false)).join('')}</div><div class="divider"></div><div class="row between"><span>Pontos de nível disponíveis</span><b>${remainingLevelPoints()} / ${earnedLevelPoints()}</b></div><div class="level-points">${[20,40,60,80,100].map(n=>`<div class="point-dot ${state.level>=n?'earned':''}">Nv ${n}</div>`).join('')}</div>${remainingLevelPoints()>0?`<div class="row" style="margin-top:10px">${ATTRS.map(([k,a])=>`<button data-lvl-inc="${k}" ${ordinaryAttrRaw(k)>=5?'disabled':''}>+1 ${a}</button>`).join('')}</div>`:''}${spentLevelPoints()>0?`<div class="row" style="margin-top:8px">${ATTRS.filter(([k])=>state.levelAttributes[k]>0).map(([k,a])=>`<button data-lvl-dec="${k}" class="ghost">−1 ${a}</button>`).join('')}</div>`:''}</article>
      </div>
      <section class="mechanics-section"><div class="section-title"><div><p class="eyebrow">PERÍCIAS & TREINAMENTO</p><h2>Rolagens, proficiência e origem</h2></div><span class="pill">Nv 20/40 pela origem · Treino: 21/41/61/81/100</span></div>${renderSkillTrainingCard()}<div style="margin-top:18px">${renderSkillMatrix()}</div></section>
      ${renderTalentsSection()}
      ${renderLegacySection()}
      <div class="grid two mechanics-section"><article class="card"><p class="eyebrow">ORIGEM DOS VALORES</p><p><span class="label">Fonte do kit</span><span class="pill">${g.source}</span></p><p><span class="label">HP</span>${g.hpBase} + CON; a cada 10 níveis +${g.hpPerDecade} + CON</p><p><span class="label">Bônus divinos</span>${bonusText(g)}</p><p><span class="label">Perícias divinas</span>${divineGranted().join(', ')||'—'}</p>${(g.notes||[]).map(n=>`<div class="notice">${n}</div>`).join('')}</article><article class="card"><p class="eyebrow">PENALIDADES ATUAIS</p><div class="status-line ${energyBand().cls}"><b>${energyBand().name}</b><span>${energyBand().text}</span></div><div class="subcard compact" style="margin-top:10px"><b>Penalidade global atual:</b> ${signed(globalD20Penalty())}<br><span class="muted">Energia ${energyRollPenalty()} · Exaustão ${exhaustionRollPenalty()} · retornos ${returnRollPenalty()} · temporário ${Number(state.tempMods.rolls)||0}</span></div><p class="muted compact" style="margin-top:10px">O estado de Sanidade e seus efeitos ficam junto ao contador de Sanidade no topo desta aba.</p></article></div>
      <article class="card mechanics-section"><p class="eyebrow">AJUSTES EXCEPCIONAIS DE ATRIBUTO</p><h2>Somente efeitos que ultrapassam o limite normal</h2><p class="muted">O total comum continua limitado a 5. Use estes campos apenas quando uma habilidade, bênção, maldição ou autorização disser explicitamente que pode ultrapassar esse limite.</p><div class="grid three compact-fields">${ATTRS.map(([k,a])=>`<label><span class="label">${a} extra</span><input type="number" min="-10" max="10" value="${Number(state.attributeExtras[k])||0}" data-attr-extra="${k}"></label>`).join('')}</div></article>
      ${renderDiagnostics()}
    </section>`;
  }
  ...
  function renderEquippedDeck(){
    const cards=[];
    const a=state.armor,t=armorType(a.type),am=material(a.material),s=state.shield,sm=material(s.material);
    if(a.equipped)cards.push(`<div class="equipment-card armor"><span class="equipment-card-index">I</span><div class="equipment-card-glyph">◈</div><div class="equipment-card-copy"><small>ARMADURA</small><b>${esc(a.name||'Armadura')}</b><span>${esc(t.name)} · ${esc(am.name)}</span></div><div class="equipment-card-stats"><span>DEF +${t.defense}</span><span>RD ${t.reduction}</span></div></div>`);
    if(s.equipped)cards.push(`<div class="equipment-card shield"><span class="equipment-card-index">II</span><div class="equipment-card-glyph">⬡</div><div class="equipment-card-copy"><small>ESCUDO</small><b>${esc(s.name||'Escudo')}</b><span>${esc(sm.name)}</span></div><div class="equipment-card-stats"><span>DEF +${shieldDefense()}</span><span>${Number(s.stakes)||0}/30</span></div></div>`);
    const weaponStart=cards.length;
    (state.weapons||[]).filter(w=>w.equipped!==false).forEach((w,idx)=>{const wm=material(w.material),wt=weaponType(w.type);cards.push(`<div class="equipment-card weapon"><span class="equipment-card-index">${weaponStart+idx+1}</span><div class="equipment-card-glyph">✦</div><div class="equipment-card-copy"><small>ARMA</small><b>${esc(w.name||'Arma')}</b><span>${esc(wt.name)} · ${esc(wm.name)}</span></div><div class="equipment-card-stats"><span>ATQ ${signed(weaponAttack(w))}</span><span>${weaponDamageFormula(w)}</span></div></div>`) });
    const desired=Math.max(4,cards.length);
    for(let i=cards.length;i<desired;i++)cards.push(`<div class="equipment-card empty"><span class="equipment-card-index">${i+1}</span><div class="equipment-card-glyph">XII</div><div class="equipment-card-copy"><small>SLOT LIVRE</small><b>Equipamento</b><span>Marque um item como equipado</span></div></div>`);
  ...
  function renderCombatEquipmentSummary(){
    ...
    return `<article class="card equipment-summary-card"><div class="section-title"><div><p class="eyebrow">EQUIPAMENTO EM USO</p><h3>Baralho equipado</h3></div><button type="button" data-go-tab="inventory">Abrir Inventário</button></div>${renderEquippedDeck()}<div class="equipment-inline-summary"><span>${a.equipped?`Armadura +${t.defense} DEF / ${t.reduction} RD`:'Sem armadura'}</span><span>${s.equipped?`Escudo +${shieldDefense()} DEF`:'Sem escudo'}</span><span>${eqWeapons.length?`${eqWeapons.length} arma(s) equipada(s)`:'Sem arma equipada'}</span></div></article>`;
  }

  function renderCombatTab(fDef){
    return `<section class="tab-pane ${state.activeTab==='combat'?'':'hidden'}" data-pane="combat">
      ${renderCombatResourceDock()}
      <article class="card death-visible-card death-priority">${renderDeath()}</article>
      <div class="grid two mechanics-section">
        <article class="card"><p class="eyebrow">ROLAGENS</p><div class="grid two"><div class="stat"><span>Defesa</span><strong>${fDef!==null?fDef:`1d20 ${signed(defenseBonus())}`}</strong><small class="muted">${fDef!==null?'Valor fixo por condição ativa':'DES + armadura + escudo + modificadores'}</small></div><div class="stat"><span>Iniciativa</span><strong>1d20 ${signed(initiativeBonus())}</strong><small class="muted">DES + modificadores</small></div><div class="stat"><span>Redução de dano</span><strong>${damageReduction()}</strong><small class="muted">Armadura + temporário</small></div><div class="stat"><span>Deslocamento perdido</span><strong>${state.exhaustion*5} m</strong><small class="muted">−5m por nível de Exaustão</small></div></div><div class="row" style="margin-top:12px"><button id="rollDefense" class="primary">Rolar Defesa</button><button id="rollInitiative">Rolar Iniciativa</button></div>${state.lastRoll?`<div class="roll-result"><span>${esc(state.lastRoll.label)}</span><b>${state.lastRoll.total}</b><small>d20 ${state.lastRoll.die}${state.lastRoll.modifier!==undefined?` ${signed(state.lastRoll.modifier)}`:''}</small></div>`:''}</article>
        ${renderCombatEquipmentSummary()}
      </div>
      <section class="mechanics-section"><div class="section-title"><div><p class="eyebrow">CONDIÇÕES & SOBREVIVÊNCIA</p><h2>Exaustão e condições</h2></div><span class="pill">Exaustão ${state.exhaustion}/6</span></div><article class="card exhaustion-wide">${renderExhaustion()}</article><article class="card" style="margin-top:12px"><h3>Condições ativas</h3><div class="conditions-grid">${system.conditions.map(c=>`<label class="condition-check ${activeCondition(c.name)?'active':''}"><input type="checkbox" data-condition="${esc(c.name)}" ${activeCondition(c.name)?'checked':''}><span><b>${c.name}</b><small>${c.mechanic}</small></span></label>`).join('')}</div></article></section>
      <section class="mechanics-section"><div class="grid two"><article class="card"><p class="eyebrow">DESCANSOS</p><div class="row"><button id="shortRest" class="primary">Descanso curto</button><button id="longRest">Descanso longo</button></div><p class="muted compact"><b>Curto:</b> +25 HP e +150 Energia. <b>Longo:</b> HP e Energia completos. Sanidade e Exaustão não são zeradas automaticamente.</p></article><article class="card"><p class="eyebrow">MODIFICADORES TEMPORÁRIOS</p><div class="grid three compact-fields"><label><span class="label">Mod. d20</span><input id="tempRolls" type="number" value="${state.tempMods.rolls}"></label><label><span class="label">Defesa</span><input id="tempDefense" type="number" value="${state.tempMods.defense}"></label><label><span class="label">Redução</span><input id="tempReduction" type="number" value="${state.tempMods.damageReduction}"></label></div></article></div></section>
    </section>`;
  }
  ...
  function bindInventory(){
    ...
    const add=byId('addInventoryItem');if(add)add.onclick=()=>{inv.items.push({id:uid('item'),name:'Novo item',qty:1,category:'geral',material:'',rune:'',notes:''});save();renderSheet()};
  ...
  function renderSheet(){
    syncCurrentCaps();const g=god(),set=effectiveAbilitySet(),fDef=fixedDefense(),h=state.history||{};
    const heroTagline=h.tagline||h.description||'Perfil, progressão, combate e história em um só lugar.';
    const heroMeta=[`${g.name}`,`nível ${state.level}`,`BP +${bp()}`,state.roma?.cohort?esc(state.roma.cohort):'',h.age?`${esc(h.age)} anos`:'',h.origin?esc(h.origin):''].filter(Boolean).join(' · ');
    sheetView.innerHTML=`
      <div class="sheet-head sheet-hero ${h.bannerUrl?'has-banner':''}">
        <div class="sheet-hero-bg"${h.bannerUrl?` style="background-image:url('${esc(h.bannerUrl)}')"`:''}></div>
        <div class="sheet-hero-mark">XII</div>
        <div class="sheet-hero-profile">
          <div class="sheet-hero-portrait ${h.portraitUrl?'has-image':''}"${h.portraitUrl?` style="background-image:url('${esc(h.portraitUrl)}')"`:''}>${h.portraitUrl?'':esc(initials(state.name||g.name))}</div>
      ...
      ${renderTabs()}
      ${renderStatusTab(g)}
      ${renderCombatTab(fDef)}
      <section class="tab-pane ${state.activeTab==='abilities'?'':'hidden'}" data-pane="abilities">${set?renderAbilities(set):'<div class="notice">Nenhum conjunto de habilidades carregado.</div>'}</section>
      ${renderInventoryTab()}
      ${renderFamiliarsTab()}
      ${renderRomaTab()}
      ${renderMagicTab()}
  ...
  function renderArmorCard(){
    ...
    return `<article class="card"><div class="row between"><p class="eyebrow">ARMADURA</p><label class="toggle-inline"><input id="armorEquipped" type="checkbox" ${a.equipped?'checked':''}> ativa</label></div><label><span class="label">Nome</span><input id="armorName" value="${esc(a.name)}"></label><div class="grid two"><label><span class="label">Tipo</span><select id="armorType">${system.armorTypes.map(x=>`<option value="${x.id}" ${a.type===x.id?'selected':''}>${x.name}</option>`).join('')}</select></label><label><span class="label">Material</span><select id="armorMaterial">${system.materials.map(x=>`<option value="${x.id}" ${a.material===x.id?'selected':''}>${x.name}</option>`).join('')}</select></label></div><div class="subcard compact"><b>${t.name}</b> · +${t.defense} Defesa · ${t.reduction} redução de dano<br><span class="muted">Material: ${m.name} · resistência ${m.unbreakable?'inquebrável':m.resistance} · referência de armadura: ${m.armorHint}</span></div>${!m.unbreakable?`<div class="row between"><span>Resistência atual <b>${res}/${m.resistance}</b></span><div class="row"><button data-armor-res="-1">−1</button><button data-armor-res="1">+1</button></div></div>`:'<div class="pill good">Material inquebrável</div>'}</article>`;
  }
  function renderShieldCard(){
    const s=state.shield,m=material(s.material),res=m.unbreakable?'∞':clamp(Number(s.resistanceCurrent)||0,0,m.resistance||0);
    return `<article class="card"><div class="row between"><p class="eyebrow">ESCUDO</p><label class="toggle-inline"><input id="shieldEquipped" type="checkbox" ${s.equipped?'checked':''}> ativo</label></div><label><span class="label">Nome</span><input id="shieldName" value="${esc(s.name)}"></label><label><span class="label">Material</span><select id="shieldMaterial">${system.materials.map(x=>`<option value="${x.id}" ${s.material===x.id?'selected':''}>${x.name}</option>`).join('')}</select></label><label><span class="label">Estacas do escudo · ${s.stakes}/30</span><input id="shieldStakes" type="range" min="0" max="30" value="${s.stakes}"></label><div class="pill ${hasTalent('mestre-escudos')?'good':''}">Mestre de Escudos: ${hasTalent('mestre-escudos')?'ativo':'não adquirido'}</div><div class="subcard compact">Bônus atual de Defesa: <b>+${shieldDefense()}</b>. Aos 30, usa metade do BP arredondada para cima; com Mestre de Escudos, BP completo.</div>${!m.unbreakable?`<div class="row between"><span>Resistência atual <b>${res}/${m.resistance}</b></span><div class="row"><button data-shield-res="-1">−1</button><button data-shield-res="1">+1</button></div></div>`:'<div class="pill good">Material inquebrável</div>'}</article>`;
  }
  ...
  function renderAbilities(set){
    ...
    return `<div class="abilities-wrap"><div class="section-title"><div><p class="eyebrow">HABILIDADES DIVINAS</p><h2>${lineageAbilityTitle()}</h2></div><div class="row"><span class="pill">${passives.length} passivas</span><span class="pill">${actives.length} ativas</span></div></div><div class="ability-columns" style="margin-top:14px"><article class="card"><p class="eyebrow">PASSIVAS</p><div class="ability-list">${passives.length?passives.map(renderAbility).join(''):'<div class="notice">Nenhuma passiva carregada.</div>'}</div></article><article class="card"><p class="eyebrow">ATIVAS</p><div class="ability-list">${actives.length?actives.map(renderAbility).join(''):'<div class="notice">Nenhuma ativa carregada.</div>'}</div></article></div></div>`;
  }

  function renderAbility(a){
    const maxSt=magicAbilityStakeMax(a),rawSt=stakesOf(a),st=Math.min(rawSt,maxSt),locked=a.type==='active'&&state.level<a.level,current=tierText(a,st),cost=a.type==='active'?a.cost:null;
    return `<div class="ability-card ${locked?'ability-locked':''}"><div class="ability-head"><div><div class="row"><b>${a.name}</b>${a.sourceGodId&&a.sourceGodId!==state.godId?`<span class="pill warn">${godById(a.sourceGodId)?.name||'Legado'}</span>`:''}${a.type==='active'?`<span class="pill">Nv ${a.level}</span><span class="pill energy-cost">${cost} EN</span>`:'<span class="pill">Passiva</span>'}${state.magic?.enabled&&maxSt===29?'<span class="pill warn">mágico · até 2ª estaca</span>':''}</div><p class="muted compact">${a.summary}</p></div></div>${a.tiers?`<div class="stake-box"><div class="row between"><span class="label">Estacas · faixa ${tierName(st)}</span><b>${st}/${maxSt}</b></div><input type="range" min="0" max="${maxSt}" value="${st}" data-stakes="${abilityKey(a)}" data-stake-max="${maxSt}"><div class="stake-quick row"><button data-stake-set="${abilityKey(a)}:0" data-stake-limit="${maxSt}">0</button><button data-stake-set="${abilityKey(a)}:16" data-stake-limit="${maxSt}">16</button>${maxSt>=30?`<button data-stake-set="${abilityKey(a)}:30" data-stake-limit="${maxSt}">30</button>`:''}</div></div>`:''}<div class="tier-current"><span class="label">Efeito atual</span><p>${current}</p></div>${a.extra&&a.tiers?`<details><summary>Observações</summary><p>${a.extra}</p></details>`:''}${a.type==='active'?`<div class="row between ability-actions">${locked?`<span class="locked">Bloqueada até o nível ${a.level}</span>`:`<button class="primary" data-use-ability="${abilityKey(a)}" data-cost="${cost}">Usar · −${cost} EN</button>`}<span class="muted compact">Energia atual: ${state.currentEnergy}</span></div>`:''}</div>`;
  }
  ...
  function bindSheet(){
    ...
    if(state.activeTab==='combat'){bindDeath()}
  ...
  function bindWeapons(){
    byId('addWeapon').onclick=()=>{const m=material('ferro-aco');state.weapons.push({id:uid('weapon'),name:'Nova arma',type:'corpo-a-corpo',attr:'for',material:'ferro-aco',stakes:0,resistanceCurrent:m.resistance,attackExtra:0,damageExtra:0,equipped:true});save();renderSheet()};
  ...
  byId('resetBtn').onclick=()=>{if(!confirm('Apagar a ficha local desta versão?'))return;localStorage.removeItem(STORAGE_KEY);LEGACY_KEYS.forEach(k=>localStorage.removeItem(k));state=defaultState();render();notify('Ficha resetada.')};
