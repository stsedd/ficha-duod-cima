(() => {
  ...
  function renderCharacterRail(g){
    ...
    return `<aside class="character-rail card">
      ...
      <div class="rail-level-row"><div><span>Nível</span><strong>${state.level}</strong></div><div><span>BP</span><strong>${signed(bp())}</strong></div><div><span>Defesa</span><strong>${signed(defenseBonus())}</strong></div><div><span>DT</span><strong>${castDT()}</strong></div></div>
  ...
  function renderTalentsCompact(){
    ...
    const owned=(state.talents||[]).map(inst=>{const def=talentDef(inst.talentId)||{name:'Talento desconhecido',description:'',manual:true};return `<div class="talent-compact-item"><div class="row between"><div><b>${def.name}</b><small>${inst.extra?'Extra':'Padrão'}${def.minLevel>1?` · Nv ${def.minLevel}+`:''}</small></div><button class="danger icon-danger" data-remove-talent="${inst.id}" title="Remover">×</button></div><p>${def.description}</p>${def.params?.length?`<div class="talent-params compact-fields">${talentParameterHtml(inst,def)}</div>`:''}<input class="talent-note-mini" data-talent-notes="${inst.id}" value="${esc(inst.notes||'')}" placeholder="nota / origem"></div>`}).join('');
    return `<article class="card talents-compact-card"><div class="section-title"><div><p class="eyebrow">TALENTOS</p><h3>Talentos atuais</h3></div><span class="pill">${used}/${earned}</span></div><div class="talent-compact-list">${owned||'<p class="muted compact">Nenhum talento selecionado.</p>'}</div><details class="compact-disclosure talent-add"><summary>Adicionar ou consultar talento</summary><div class="talent-picker"><select id="newTalentDef"><option value="">Escolha um talento…</option>${eligible.map(t=>`<option value="${t.id}" ${state.talentDraftId===t.id?'selected':''}>${t.name}${t.minLevel>1?` · Nv ${t.minLevel}+`:''}</option>`).join('')}</select><div id="talentPreview">${talentPreviewHtml(preview)}</div><div class="row"><button id="addStandardTalent" class="primary" ${used>=earned||!preview?'disabled':''}>Adicionar padrão</button><button id="addExtraTalent" ${!preview?'disabled':''}>Adicionar extra</button></div></div></details>${hasTalent('pau-toda-obra')?`<div class="notice compact">Pau pra Toda Obra: +${talentCount('pau-toda-obra')}d4 em Testes de Perícia.</div>`:''}</article>`;
  }
  ...
  function renderEquippedDeck(){
    ...
    if(a.equipped)cards.push(`<div class="equipment-card armor"><span class="equipment-card-index">I</span><button class="equipment-card-edit" data-edit-equipment="armor" type="button">Editar</button><div class="equipment-card-glyph">◈</div><div class="equipment-card-copy"><small>ARMADURA</small><b>${esc(a.name||'Armadura')}</b><span>${esc(t.name)} · ${esc(am.name)}</span></div><div class="equipment-card-stats"><span>DEF +${t.defense}</span><span>RD ${t.reduction}</span></div></div>`);
    if(sh.equipped)cards.push(`<div class="equipment-card shield"><span class="equipment-card-index">II</span><button class="equipment-card-edit" data-edit-equipment="shield" type="button">Editar</button><div class="equipment-card-glyph">⬡</div><div class="equipment-card-copy"><small>ESCUDO</small><b>${esc(sh.name||'Escudo')}</b><span>${esc(sm.name)}</span></div><div class="equipment-card-stats"><span>DEF +${shieldDefense()}</span><span>${Number(sh.stakes)||0}/30</span></div></div>`);
    const weaponStart=cards.length;
    (state.weapons||[]).filter(w=>w.equipped!==false).forEach((w,idx)=>{const wm=material(w.material),wt=weaponType(w.type);cards.push(`<div class="equipment-card weapon"><span class="equipment-card-index">${weaponStart+idx+1}</span><button class="equipment-card-edit" data-edit-equipment="weapon:${w.id}" type="button">Editar</button><div class="equipment-card-glyph">✦</div><div class="equipment-card-copy"><small>ARMA</small><b>${esc(w.name||'Arma')}</b><span>${esc(wt.name)} · ${esc(wm.name)}</span></div><div class="equipment-card-stats"><span>ATQ ${signed(weaponAttack(w))}</span><span>${weaponDamageFormula(w)}</span></div></div>`) });
    const desired=Math.max(4,cards.length);
    for(let i=cards.length;i<desired;i++)cards.push(`<div class="equipment-card empty"><span class="equipment-card-index">${i+1}</span><button class="equipment-card-edit" data-go-tab="inventory" type="button">Adicionar</button><div class="equipment-card-glyph">XII</div><div class="equipment-card-copy"><small>SLOT LIVRE</small><b>Equipamento</b><span>Marque um item como equipado</span></div></div>`);
  ...
  function renderSkillMatrix(){
    return `<article class="card skills-ledger-card"><div class="row between skills-ledger-head"><div><p class="eyebrow">PERÍCIAS</p><h3>Todas as perícias</h3></div><span class="pill">BP +${bp()}</span></div><div class="skill-ledger">${skills.map(sk=>{
      ...
      return `<div class="skill-ledger-row ${trained?'trained':''} ${expert?'expert':''}"><div class="skill-row-main"><div class="skill-ledger-name"><b>${sk.name}</b><small>${attrName(sk.attr)}${fixed?` · kit ${signed(fixed)}`:''}</small></div><strong class="skill-roll-value">${signed(skillValue(sk.name))}</strong></div><div class="skill-ledger-toggles"><label class="skill-mini-toggle ${trained?'active':''}" title="Perito"><input type="checkbox" data-skill-manual="${esc(sk.name)}" ${trained?'checked':''} ${automatic.length?'disabled':''}><span>P</span></label><label class="skill-mini-toggle ${expert?'active expert':''}" title="Expertise"><input type="checkbox" data-skill-expertise="${esc(sk.name)}" ${expert?'checked':''} ${trained?'':'disabled'}><span>E</span></label><details class="skill-settings"><summary>origem</summary><div class="skill-settings-body">${sourceHtml}<input class="skill-detail-inline" data-skill-detail="${esc(sk.name)}" value="${esc(meta.detail||'')}" placeholder="Detalhe / origem"></div></details></div></div>`;
  ...
  function renderAbility(a){
    ...
    return `<article class="ability-card ${locked?'ability-locked':''} ${a.type==='active'?'active-ability':'passive-ability'}"><div class="ability-head"><div><div class="row ability-title-row"><b>${a.name}</b>${a.sourceGodId&&a.sourceGodId!==state.godId?`<span class="pill warn">${godById(a.sourceGodId)?.name||'Legado'}</span>`:''}</div><div class="ability-meta">${a.type==='active'?`<span>Nv ${a.level}</span><span>${cost} EN</span>`:'<span>Passiva</span>'}${state.magic?.enabled&&maxSt===29?'<span>mágico · até 29</span>':''}</div></div></div><p class="ability-summary">${a.summary}</p>${tiers.length?`<div class="ability-tier-list">${tiers.map(([key,label,text,current])=>`<div class="ability-tier ${current?'current':''}" data-tier="${key}"><b>${label}</b><p>${text}</p></div>`).join('')}</div><div class="stake-compact"><div class="row between"><span>Estacas</span><b>${st}/${maxSt}</b></div><input type="range" min="0" max="${maxSt}" value="${st}" data-stakes="${abilityKey(a)}" data-stake-max="${maxSt}"><div class="stake-quick"><button data-stake-set="${abilityKey(a)}:0" data-stake-limit="${maxSt}">0</button><button data-stake-set="${abilityKey(a)}:16" data-stake-limit="${maxSt}">16</button>${maxSt>=30?`<button data-stake-set="${abilityKey(a)}:30" data-stake-limit="${maxSt}">30</button>`:''}</div></div>`:`<div class="ability-tier-list single"><div class="ability-tier current"><p>${a.extra||'Sem estacas mecânicas cadastradas.'}</p></div></div>`}${a.extra&&a.tiers?`<details class="ability-notes"><summary>Observações</summary><p>${a.extra}</p></details>`:''}${a.type==='active'?`<div class="ability-actions">${locked?`<span class="locked">Bloqueada até o nível ${a.level}</span>`:`<button class="primary" data-use-ability="${abilityKey(a)}" data-cost="${cost}">Usar · −${cost} EN</button>`}<small>EN atual ${state.currentEnergy}</small></div>`:''}</article>`;
  }
