(() => {
  ...
  function renderCombatTab(fDef){
    ...
    return `<section class="tab-pane ${state.activeTab==='combat'?'':'hidden'}" data-pane="combat">
      ...
      <div class="combat-focus-grid mechanics-section">
        <article class="card combat-essentials-card"><p class="eyebrow">FOCO DE COMBATE</p><div class="combat-essentials-grid"><div class="stat"><span>Sanidade</span><strong>${state.currentSanity}/100</strong><small>${san.name} · ${san.text}</small></div><div class="stat"><span>Defesa</span><strong>${fDef!==null?fDef:`1d20 ${signed(defenseBonus())}`}</strong><small>${fDef!==null?'valor fixo por condição ativa':'rolagem baseada em DES + armadura + escudo'}</small></div><div class="stat"><span>Energia</span><strong>${state.currentEnergy}/${energyMax()}</strong><small>${energy.name}</small></div><div class="stat"><span>Redução de dano</span><strong>${damageReduction()}</strong><small>armadura + temporário</small></div></div><div class="grid two compact-fields" style="margin-top:12px"><label><span class="label">Modificador temporário de Defesa</span><input id="tempDefense" type="number" value="${state.tempMods.defense}"></label><label><span class="label">Modificador global de rolagem</span><input id="tempRolls" type="number" value="${state.tempMods.rolls}"></label></div>${state.lastRoll?`<div class="roll-result"><span>${esc(state.lastRoll.label)}</span><b>${state.lastRoll.total}</b><small>d20 ${state.lastRoll.die}${state.lastRoll.modifier!==undefined?` ${signed(state.lastRoll.modifier)}`:''}</small></div>`:''}<div class="row" style="margin-top:12px"><button id="rollDefense" class="primary">Rolar Defesa</button></div></article>
        ${renderCompactActiveConditions()}
      </div>
      <article class="card death-visible-card death-priority mechanics-section">${renderDeath()}</article>
      <div class="grid two mechanics-section">
        ${renderCombatEquipmentSummary()}
        <article class="card"><p class="eyebrow">DESCANSOS & MODS</p><div class="row"><button id="shortRest" class="primary">Descanso curto</button><button id="longRest">Descanso longo</button></div><p class="muted compact"><b>Curto:</b> +25 HP e +150 Energia. <b>Longo:</b> HP e Energia completos. Sanidade e Exaustão não são zeradas automaticamente.</p><div class="grid two compact-fields" style="margin-top:12px"><label><span class="label">Redução temporária de dano</span><input id="tempReduction" type="number" value="${state.tempMods.damageReduction}"></label><label><span class="label">Exaustão</span><select id="combatExhaustionSelect">${[0,1,2,3,4,5,6].map(n=>`<option value="${n}" ${state.exhaustion===n?'selected':''}>${n}</option>`).join('')}</select></label></div><div class="subcard compact" style="margin-top:12px"><b>Efeito atual da Exaustão:</b> ${state.exhaustion?`${exhaustionRollPenalty()} em todos os d20 e −${state.exhaustion*5}m de deslocamento.`:'sem penalidade.'}${state.exhaustion>=5?' No nível 5, HP e Energia máximos ficam pela metade.':''}${state.exhaustion>=6?' <strong>Exaustão 6 significa morte.</strong>':''}</div></article>
      </div>
    </section>`;
  }
